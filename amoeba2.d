...
{
	return a > b ? a - b < am_Float.epsilon : b - a < am_Float.epsilon;
}
...
pragma(inline, true)
{
	...
	auto am_offset(L, R)(L lhs, R rhs) ( return (int)((char*)(lhs) - (char*)(rhs)))
	auto am_index(h, i)      ((am_Entry*)((char*)(h) + (i)))
}
...
static am_Entry *am_settable(am_Solver *solver, am_Table *t, am_Symbol key) {
	am_Entry *e;
	assert(key.id != 0);
	if ((e = (am_Entry*)am_gettable(t, key)) !is null) return e;
...
static void am_delta_edit_constant(am_Solver *solver, am_Float delta, am_Constraint *cons) {
	am_Row *row;
	if ((row = (am_Row*)am_gettable(&solver.rows, cons.marker)) !is null)
	{ if ((row.constant -= delta) < 0.0f) am_infeasible(solver, row); return; }
	if ((row = (am_Row*)am_gettable(&solver.rows, cons.other)) !is null)
	{ if ((row.constant += delta) < 0.0f) am_infeasible(solver, row); return; }
	while (am_nextentry(&solver.rows, (am_Entry**)&row)) {
...
int am_addedit(am_Variable *var, am_Float strength) {
	...
	am_Constraint *cons;
	if (var == null || var.constraint !is null) return AM_FAILED;
	assert(var.sym.id != 0);
